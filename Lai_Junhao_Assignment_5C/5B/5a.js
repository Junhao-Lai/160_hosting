...
function loadwork() {
...

    {
        ...
        mtlLoader.load('./models/cup/champions-league.mtl', (mtl) => {
            mtl.preload();
            const objLoader = new OBJLoader();
            mtl.materials.Material.side = THREE.DoubleSide;
            objLoader.setMaterials(mtl);
            objLoader.load('./models/cup/champions-league.obj', (root) => {
                //scene.position.y = 9;
                scene.add(root);
            });
        });
    ...
    {
        const mtlLoader = new MTLLoader();
        mtlLoader.load('./models/ball/soccer-ball.mtl', (mtl) => {
            mtl.preload();
            const objLoader = new OBJLoader();
            objLoader.setMaterials(mtl);
            objLoader.load('./models/ball/soccer-ball.obj', (root) => {

                scene.add(root);

                root.position.set(10, -0.1, 9);
                const box = new THREE.Box3().setFromObject(root);
...
{
    const mtlLoader = new MTLLoader();
    mtlLoader.load('./models/cone2/cone.mtl', (mtl) => {
        mtl.preload();
        const objLoader = new OBJLoader();
        objLoader.setMaterials(mtl);
        objLoader.load('./models/cone2/cone.obj', (root) => {

            scene.add(root);

            // Scale the cone down
            root.scale.set(0.3, 0.3, 0.3); // Adjust the scale as needed (0.5 means half the size)

            root.position.set(5, -0.1, -9);
            const box = new THREE.Box3().setFromObject(root);
            ...
            function frameArea(sizeToFitOnScreen, boxSize, boxCenter, camera) {
                ...
                // pick some near and far values for the frustum that
                // will contain the box.
                camera.near = boxSize / 25;
                camera.far = boxSize * 35;
            ...
            // set the camera to frame the box
            frameArea(boxSize * 20, boxSize, boxCenter, camera);
...
//cone creation #2
{
    ...
    //sub creation #1
    const mtlLoader = new MTLLoader();
    mtlLoader.load('./models/sub/Soccerbench.mtl', (mtl) => {
        mtl.preload();
        const objLoader = new OBJLoader();
        objLoader.setMaterials(mtl);
        objLoader.load('./models/sub/Soccerbench.obj', (root) => {
            scene.add(root);

            // Scale down the object
            const scaleFactor = 0.3; // Adjust this value as needed

            root.scale.set(scaleFactor, scaleFactor, scaleFactor);

            root.rotation.y = Math.PI / 2;

            root.position.set(7, -0.1, 0);
            const box = new THREE.Box3().setFromObject(root);
    ...
    // makePerson(-0.8, 1, 150, 32, 'Middle', 'purple');
